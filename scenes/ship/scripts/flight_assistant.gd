...
func apply_controls(delta: float):
	...
	if enabled:
		if _is_tracking:
			...
			_move_to(delta, _target_position)
		else:
			_handle_turn(delta, _pointer_position)
	_handle_control()
...
func _get_delta_velocity(accurate: bool = false) -> Vector2:
	if is_instance_valid(target):
		...
		if _tracking_accuracy <= LINEAR_THRESHOLD:
			return delta
		var delta_l = delta.length()
		if delta_l > _tracking_accuracy:
			var d = max(0.0, delta_l - _tracking_accuracy if not accurate else 0.0)
