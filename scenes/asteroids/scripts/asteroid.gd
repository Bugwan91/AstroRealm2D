...
func init(
	...
	spd_variation: float,
	rot: float,
	rot_variation: float) -> void:
		reset()
		mass = _base_mass * _size * _size
		...
		linear_velocity = vel + _rand_velocity(_speed, spd_variation, size_inv)
...
func _rand_velocity(_speed: float, variation: float, size_inv: float) -> Vector2:
	return  (_speed * size_inv * randf_range(1.0 - variation, 1.0 + variation) * Vector2.ONE).rotated(randf_range(-PI, PI))

func _rand_rotation(_speed: float, variation: float, size_inv: float) -> float:
	return _speed * size_inv * randf_range(-(1.0 + variation), 1.0 + variation)
