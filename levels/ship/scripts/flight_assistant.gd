...
func _get_configuration_warnings():
	...
	if not (is_instance_valid(ship) and ship is RigidBody2D):
		warnings.append("Needs rigidbody to control")
	if not (is_instance_valid(input_reader) and input_reader is ShipInputReader):
		warnings.append("Needs input source")
	if not (is_instance_valid(thrusters) and thrusters is Thrusters):
		warnings.append("Needs thrusters to operate")
	if not (is_instance_valid(engines) and engines is Engines):
		warnings.append("Needs engines to operate")
	return warnings
