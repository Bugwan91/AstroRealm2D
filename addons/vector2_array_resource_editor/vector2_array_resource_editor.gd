...
func _edit(object):
	if object is Vector2ArrayResource:
		...
		update_overlays()
		_editable.changed.connect(update_overlays)
	else:
		_clear_editable()
...
func _get_active_vertex() -> int:
	for index in range(0, _editable.data.size()):
		# vertex is active when distance from cursor to it is less than treshold
		if (_cursor - _transform_to_view * _editable.data[index]).length() < CURSOR_THRESHOLD:
			return index
	return -1
...
func _get_active_side() -> int:
	...
	if _editable.active_index != -1:
		return -1
	var size := _editable.data.size()
	for index in range(0, size):
		# checking if cursor position is between polygon side vertexes
		var a := _transform_to_view * _editable.data[index]
		var b := _transform_to_view * _editable.data[index + 1 if index + 1 < size else 0]
		var ab = (b - a).length()
...
func _remove_vertex():
	...
	if _editable.active_index == -1 or _editable.data.size() < 4:
		return
	var undo := get_undo_redo()
...
func _draw_polygon(overlay: Control):
	if not is_instance_valid(_editable):
		return
	overlay.draw_colored_polygon(_transform_to_view * _editable.data, POLYGON_COLOR)
	for index in range(_editable.data.size()):
		_draw_vertex(overlay, _transform_to_view * _editable.data[index], index)
	if _can_add_at != -1:
